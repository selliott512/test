...
```shell
unzip wad-tweak-0.9.0.zip
wad-tweak-0.9.0/bin/wad-to-lump.py ...
wad-tweak-0.9.0/bin/wad-shuffle-dir.py ...
```
...
```shell
unzip -d /opt wad-tweak-0.9.0.zip # Requires root.
PATH="$PATH:/opt/wad-tweak-0.9.0/bin"
wad-to-lump.py ...
wad-shuffle-dir.py ...
```
...
```shell
wad-to-lump.py -vip comcon.wad \_standard\_
```
...
Doom WADs sometimes have namespaces marked by empty lumps with pattern <namespace>\_START and <namespace>\_END. When the namespace option (-n) is passed the namespace will be indicated for show (-s), and subdirectories will be created for the namespace when writing to a directory (-d). For example, to see only the namespace markers (\_ns\_ group) in freedoom2.wad as well as the implied namespace (-n):
```shell
wad-to-lump.py -lvins freedoom2.wad \_ns\_
```
...
```
For example, when writing to a directory (-d) lumps between "F3\_START" and "F3\_END" would end up in subdirectory "f/f3" relative to the output directory (-d). Notice the "NS" column.
